...
#define SHARED_MEM_NAME "/shared_mem"// shared mem to store ings.
#define SEMAPHORE_MEM_NAME "/semaphore_mem"// shared memory to store semaphore.

sig_atomic_t sigintcaught = 0;
static sem_t *sp;// semaphore pointer.
int childPidsArr[6];
...
int main(int argc, char **argv){
    ...
    int inpfd = 0,bytesread = 0,chefCount = 6,forkVal = -1,allDeserts = 0,sharedMemFd,semaphoreMemFd,pid = getpid(),status = 0;
    ...
    if (argc < 3){
        ...
        message = (char*) malloc(MESSAGE_LEN * sizeof(char));
        sprintf(message,"Usage: vg ./hw3unnamed -i inputFilePath\n");
        printMessage(message);
        exit(EXIT_FAILURE);
    }else if (argc > 3){
    ...
    // unnamedsemaphore settings: START
    semaphoreMemFd = shm_open(SEMAPHORE_MEM_NAME,O_RDWR | O_CREAT,0666);
    if (semaphoreMemFd == -1){
        perror("semaphoreMemFd");
        close(inpfd);
        exit(EXIT_FAILURE);
    }
    if (ftruncate(semaphoreMemFd,1) == -1){
        perror("ftruncate");
        close(semaphoreMemFd);
        close(inpfd);
        exit(EXIT_FAILURE);
    }
    sp = mmap(NULL,1,PROT_READ | PROT_WRITE,MAP_SHARED,semaphoreMemFd,0);
    if (sp == MAP_FAILED){
        perror("addr");
        close(semaphoreMemFd);
        close(inpfd);
    ...
    if (close(semaphoreMemFd) == -1){
        perror("close");
        close(inpfd);
        exit(EXIT_FAILURE);
    }
    if (sem_init(sp, 1, semVal) == -1){
        perror("Failed to sem_init operation!\n");
        close(sharedMemFd);
        close(inpfd);
        exit(EXIT_FAILURE);
    }
    // unnamedsemaphore settings: END
    ...
    for (int i = 0; i < chefCount; ++i){// collect the child process return values.
        forkVal = fork();
        if (forkVal == -1){
            perror("fork:");
            close(sharedMemFd);
